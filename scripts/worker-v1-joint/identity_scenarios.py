...
from gateway_fixture import SCOPE
...
def await_identity(h, identity):
    ...
    def registered():
        return bool(rows(h, "SELECT a.operation_id FROM gateway.gateway_telegram_registrations r "
                         "JOIN gateway.gateway_telegram_registration_attempts a ON a.operation_id=r.operation_id "
                         "WHERE r.account_id=" + h.quote(identity["account_id"]) + " AND a.state='FINISHED' AND a.result='READY'"))
    h.wait(registered, "actual SDK getMe/setWebhook qualifies selected bot", timeout=40)
    return view
