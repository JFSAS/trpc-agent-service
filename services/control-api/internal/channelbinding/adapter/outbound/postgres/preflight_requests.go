...
func (t *preflightTx) SaveRequest(ctx context.Context, r application.PreflightRequest) error {
	...
	if r.PreflightID != "" {
		...
		if r.Kind == "create" {
			...
			var grant channelv1.PreflightGrant
			if json.Unmarshal(r.Response, &grant) != nil || stored.LeaseExpiresAt == nil || stored.View.GatewayConfigDigest == nil || stored.PrincipalID != r.PrincipalID || stored.InstanceID != r.InstanceID || stored.InstanceEpoch != r.InstanceEpoch || grant.SourceEpoch != stored.SourceEpoch || grant.Provider != stored.View.Provider || grant.ProviderAccountID != stored.View.ProviderAccountID || grant.AccountRevision != stored.View.AccountRevision || grant.ConnectionRevision != stored.View.ConnectionRevision || grant.WebhookPath != stored.WebhookPath || grant.Credentials.CredentialID != stored.CredentialID || grant.Credentials.CredentialVersion != stored.View.BotTokenVersion || grant.Credentials.Configured != stored.BotTokenConfigured || grant.WebhookSecretConfigured != stored.WebhookSecretConfigured || grant.LeaseEpoch != stored.LeaseEpoch || !grant.LeaseExpiresAt.Equal(*stored.LeaseExpiresAt) || !grant.JobDeadlineAt.Equal(stored.View.JobDeadlineAt) || grant.GatewayConfigDigest != *stored.View.GatewayConfigDigest {
				return integrity()
			}
		}
	}
