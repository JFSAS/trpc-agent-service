...
func validateAttachments(items []Attachment) error {
	...
	for _, a := range items {
		...
		if a.Name == "" || a.Name == "." || a.Name == ".." || len(a.Name) > 255 || !utf8.ValidString(a.Name) || strings.ContainsAny(a.Name, `/\`) || strings.IndexFunc(a.Name, unicode.IsControl) >= 0 || a.Version < 0 || a.Version > MaxExactInteger || a.SizeBytes < 0 || a.SizeBytes > MaxExactInteger || len(a.MIMEType) > 255 || mimeErr != nil || !strings.Contains(media, "/") || strings.ContainsAny(a.MIMEType, "\r\n") || !attachmentHash.MatchString(a.SHA256) {
			return ErrInvalid
		}
		if seen[a.Name] == nil {
