...
func TestGatewayV1DatabaseRoleContract(t *testing.T) {
	...
	for range 2 {
		...
		func() {
			...
			var migrations int
			// Reply receipts and receive modes retain distinct full-filename identities.
			if err := pool.QueryRow(ctx, "SELECT count(*) FROM gateway_schema_migrations").Scan(&migrations); err != nil || migrations != 12 {
