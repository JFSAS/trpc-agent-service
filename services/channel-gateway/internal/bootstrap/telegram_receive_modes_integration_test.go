...
func TestTelegramReceiveModesRealHTTPPGNATS(t *testing.T) {
	...
	for _, origin := range []string{"", "https://gateway.example"} {
		t.Run(fmt.Sprintf("origin=%s", origin), func(t *testing.T) {
			...
			config := Config{AccountSource: "control", Worker: WorkerConfig{URL: cfg.URL, CAFile: cfg.CAFile, CertificateFile: cfg.CertificateFile, KeyFile: cfg.KeyFile}, Control: cfg, InstanceID: "gw", HTTPAddress: "127.0.0.1:0", AdminAddress: "localhost:0", DatabaseURL: database.runtimeURL, MigrationDatabaseURL: database.migrationURL, NATSURL: natsURL, Topology: topology, telegramFactory: httpReceiverFactory{api.URL}}
