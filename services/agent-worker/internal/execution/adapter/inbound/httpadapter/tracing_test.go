...
func TestProofTraceInheritedOnlyAfterGatewayAuthorization(t *testing.T) {
	...
	for _, allowed := range []bool{false, true} {
		ids := []string{controlID}
		if allowed {
			ids = []string{gatewayID}
		}
		r := request(proof.FinalVerifyPath, finalBody(), ids...)
	...
	if f.calls != 1 || len(spans) != 1 || spans[0].Name != "worker.reply.verify" || spans[0].Parent.SpanID() != parent.SpanID() || spans[0].SpanKind != trace.SpanKindServer {
