...
func TestQuotaWaitRecordsMetricWithoutRoutineLog(t *testing.T) {
	for _, operation := range []string{"quota_reconcile", "consumption_reconcile"} {
		t.Run(operation, func(t *testing.T) { testAccountingWaitMetric(t, operation) })
	}
}
func testAccountingWaitMetric(t *testing.T, name string) {
	reader := sdkmetric.NewManualReader()
	var logs bytes.Buffer
	r, err := newRecorder("quota-test", &logs, reader)
	if err != nil {
		t.Fatal(err)
	}
	ctx := context.Background()
	r.Observe(ctx, app.Observation{Operation: name, Result: "quota_wait", Duration: time.Millisecond})
	var data metricdata.ResourceMetrics
	if err = reader.Collect(ctx, &data); err != nil {
		t.Fatal(err)
	}
			...
			if sum, ok := m.Data.(metricdata.Sum[int64]); ok {
				for _, point := range sum.DataPoints {
					...
					for _, a := range point.Attributes.ToSlice() {
						if a.Key == "operation" {
							operation = a.Value.AsString()
						}
						if a.Key == "result" {
							result = a.Value.AsString()
						}
					}
					if operation == name && result == "quota_wait" && point.Value == 1 {
						found = true
					}
				}
			}
	...
	if !found {
		t.Fatal("quota wait disappeared into generic labels")
	}
	if err = r.Shutdown(ctx); err != nil {
		t.Fatal(err)
	}
	if logs.Len() != 0 {
		t.Fatalf("routine hold flooded log: %s", logs.String())
	}
}
