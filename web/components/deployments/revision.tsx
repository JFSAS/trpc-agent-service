...
export function DeploymentRevisionDetail({ tenantId, deploymentId, revisionNumber }: { tenantId: string; deploymentId: string; revisionNumber: number }) {
  ...
  return <div className={styles.page}><PageHeader eyebrow="IMMUTABLE DEPLOYMENT REVISION" title={`${name} · r${revisionNumber}`} description="这是固定的发布快照。历史版本不随 Agent 或 Profile 的最新版本变化。" action={<div className={styles.actions}>{channelHref && <Link className="button primary" href={channelHref}>接入渠道</Link>}<Link className="button secondary" href={deploymentHref(tenantId, deploymentId)}>返回部署工作台</Link></div>} />
