...
export function ProfileResourceEditor({ tenantId = "", config, credentials, credentialStates, onChange, isOwner, disabled = false, readOnly = false, focusTarget }: ProfileResourceEditorProps) {
  ...
  if (name && category === "models") {
    ...
    form = <>
      {validRole ? <ManagedBackendSelect tenantId={tenantId} role={name as "session" | "memory" | "artifact"} value={storage} disabled={disabled} readOnly={readOnly} onChange={(selection) => updateResource({ ...storage, ...selection })} /> : <p role="alert">Managed Storage 类型必须与固定角色名 session、memory、artifact 一致。</p>}
      <p>平台托管资源不填写连接目标或平台凭据。跨类型替换请通过既有删除确认流程后重新创建，不自动迁移凭据。</p>
    </>;
