...
export function ProfileResourceEditor({ tenantId = "", config, credentials, credentialStates, onChange, isOwner, disabled = false, readOnly = false, focusTarget }: ProfileResourceEditorProps) {
  ...
  useEffect(() => {
    ...
    const credentialFields: Record<string, string> = { api_key_credential_id: "api_key", bearer_token_credential_id: "bearer_token", qdrant_api_key_credential_id: "qdrant_api_key", dsn_credential_id: category === "storage" && name === "memory" && own(config.storage, name)?.kind === "managed_memory" ? "dsn_password" : "dsn" };
