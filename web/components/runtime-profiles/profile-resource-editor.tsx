...
export function ProfileResourceEditor({ config, credentials, credentialStates, onChange, isOwner, disabled = false, readOnly = false, focusTarget }: ProfileResourceEditorProps) {
  const prefix = useId();
  ...
  const addError = countError || (newName && !namePattern.test(newName) ? "名称以小写字母开头，仅含小写字母、数字、_ 或 -，最多 64 个字符" : "") || (newName && Object.prototype.hasOwnProperty.call(config[category] ?? {}, newName) ? "当前分类中已存在同名资源" : "");
  ...
  useEffect(() => {
    ...
    const credentialFields: Record<string, string> = { api_key_credential_id: "api_key", bearer_token_credential_id: "bearer_token", qdrant_api_key_credential_id: "qdrant_api_key", dsn_credential_id: "dsn" };
  ...
  const navigate = (nextCategory: ResourceCategory, nextName = Object.keys(config[nextCategory] ?? {})[0] ?? "") => { setCategory(nextCategory); setChosenName(nextName); setNewName(""); setDeleteTarget(null); };
    ...
    <aside className={styles.navigation} aria-label="资源导航">
      ...
      {!readOnly && <section className={styles.addResource}>
        ...
        <Button type="button" variant="secondary" disabled={disabled || !newName || !!addError} onClick={() => {
          ...
          onChange({ ...config, [category]: { ...config[category], [newName]: initialResource(category) } }, credentials);
