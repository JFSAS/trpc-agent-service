...
export function WorkspaceFields({ node, executors, replace, disabled }: {
  ...
  return <fieldset disabled={disabled} style={{ display: "grid", gap: 8, minWidth: 0 }}>
    ...
    <small>先在 Requirements / Executors 声明槽位，再在此逐项选择工具。未勾选工具不启用；取消最后一项会移除 workspace 声明。模型需声明 tool_call。</small>
    <small>workspace_exec 在 SDK Sandbox 工作区执行；workspace_save_artifact 将工作区文件保存到已配置的 Artifact 存储。这里不填写主机路径、URL 或环境秘密。</small>
