...
import {
  channelApi, channelError, ChannelApiError, credentialPurposes, isChannelUncertain, validateCredential,
  type ChannelAccountDetails, type ChannelTarget, type CredentialPurpose, type ChannelCommandResult,
...
type Action = { kind: "account"; enabled: boolean } | { kind: "binding"; enabled: boolean } | { kind: "target"; target: ChannelTarget } | { kind: "credential"; purpose: CredentialPurpose; action: "replace" | "clear"; value?: string };
...
export function ChannelWorkspace({ tenantId, accountId, query = "", initialPreflightId, invalidPreflightQuery = false }: { tenantId: string; accountId: string; query?: string; initialPreflightId?: string; invalidPreflightQuery?: boolean }) {
  ...
  useEffect(() => {
    secretValue.current = ""; setCredentialValue(""); setRetryValue(""); setCredentialPurpose(null); setPrepared(null);
  ...
  async function openAction(action: Action) {
    ...
    if (mounted.current && snapshot) {
      if (action.kind === "binding" && action.enabled && (!snapshot.account.enabled || credentialPurposes(snapshot.account.provider).some((p) => !snapshot.account.credentials.find((c) => c.purpose === p)?.configured))) setError("请先启用本平台接入并配置全部必需凭据，再开启消息路由。");
  ...
  async function execute(p: ChannelPending, value = secretValue.current): Promise<ChannelCommandResult> {
    ...
    if (p.operation === "updateAccount") return channelApi.updateAccount(tenantId, accountId, { expected_account_revision: Number(i.expected_account_revision), ...(typeof i.name === "string" ? { name: i.name } : {}), ...(typeof i.description === "string" ? { description: i.description } : {}) }, key);
  ...
  async function submit(p: ChannelPending, value?: string) {
    ...
    try {
      ...
      secretValue.current = ""; setRetryValue(""); setCredentialPurpose(null); setMetaEditing(false); setPending(null); setPendingBlocked(false);
      ...
      if (mounted.current) { if (fresh) setNotice(p.operation === "updateAccount" ? "名称与描述已保存。" : "配置已保存。接入意图、路由分发和连接观测分别更新；这不表示 Agent 已运行或已回复。"); else { setAccepted(true); setNotice("操作已接受，最新状态读取失败。请仅重试读取，不要再次提交。"); } }
    } catch (e) {
      if (!mounted.current) return;
  ...
  async function finishPending() {
    ...
    const fresh = await refresh();
    if (fresh && mounted.current) { try { clearChannelPending(sessionStorage, pendingKey); } catch { /* Explicit ending is kept in this page too. */ } setPending(null); setPendingBlocked(false); secretValue.current = ""; setRetryValue(""); if (pending?.operation === "updateAccount") {
  ...
  return <div className={styles.page}>
    ...
    {!data ? !error && <p role="status">正在读取渠道账户…</p> : <>
      ...
      {data.account.provider === "telegram" && tab !== "diagnostics" && <div className={styles.notice}><div className={styles.row}><div><strong>启用前先检查接入条件</strong><p className={styles.hint}>预检只读取 Telegram 信息，不修改 Webhook，也不要求先配置运行目标。</p></div><Button variant="secondary" onClick={() => setTab("diagnostics")}>查看接入预检</Button></div></div>}
...
function AccountFacts({ data }: { data: ChannelAccountDetails }) { const a = data.account; return <dl className={styles.facts}><dt>Provider / 物理身份</dt><dd>{a.provider} / {a.provider_account_id}</dd><dt>账户 ID</dt><dd className={styles.mono}>{a.account_id}</dd><dt>{a.provider === "telegram" ? "Webhook 路径（只读）" : "Bot ID（只读）"}</dt><dd className={styles.mono}>{a.provider === "telegram" ? a.config.webhook_path ?? "未返回" : a.config.bot_id ?? "未返回"}</dd></dl>; }
function actionTitle(a: Action) { return a.kind === "account" ? a.enabled ? "启用本平台接入？" : "停用本平台接入？" : a.kind === "binding" ? a.enabled ? "开启消息路由？" : "暂停消息路由？" : a.kind === "target" ? "保存固定部署目标？" : a.action === "replace" ? "替换此项凭据？" : "清除此项凭据？"; }
