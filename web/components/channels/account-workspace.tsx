...
import {
  ...
  type ChannelAccountDetails, type TelegramReceiveMode, type ChannelTarget, type CredentialPurpose, type ChannelCommandResult,
...
type Action = { kind: "mode"; mode: TelegramReceiveMode } | { kind: "account"; enabled: boolean } | { kind: "binding"; enabled: boolean } | { kind: "target"; target: ChannelTarget } | { kind: "credential"; purpose: CredentialPurpose; action: "replace" | "clear"; value?: string };
...
export function ChannelWorkspace({ tenantId, accountId, query = "", initialPreflightId, invalidPreflightQuery = false }: { tenantId: string; accountId: string; query?: string; initialPreflightId?: string; invalidPreflightQuery?: boolean }) {
  ...
  async function openAction(action: Action) {
    ...
    if (mounted.current && snapshot) {
      ...
      else if (action.kind === "mode" && action.mode === snapshot.account.config.receive_mode) { setModeEditing(false); setNotice("接收方式未变化，本次未提交。"); }
  ...
  function makePending(p: Prepared): ChannelPending {
    ...
    switch (a.kind) {
      case "mode": operation = "updateAccount"; input = { expected_account_revision: d.account.account_revision, config: { receive_mode: a.mode } }; break;
  ...
  async function execute(p: ChannelPending, value = secretValue.current): Promise<ChannelCommandResult> {
    ...
    if (p.operation === "updateAccount") return channelApi.updateAccount(tenantId, accountId, { expected_account_revision: Number(i.expected_account_revision), ...(typeof i.name === "string" ? { name: i.name } : {}), ...(typeof i.description === "string" ? { description: i.description } : {}), ...(i.config ? { config: i.config as { receive_mode: TelegramReceiveMode } } : {}) }, key);
  ...
  async function finishPending() {
    ...
    if (fresh && mounted.current) { try { clearChannelPending(sessionStorage, pendingKey); } catch { /* Explicit ending is kept in this page too. */ } setPending(null); setPendingBlocked(false); secretValue.current = ""; setRetryValue(""); if (pending?.operation === "updateAccount" && pending.input.config) {
        setNextMode((pending.input.config as { receive_mode: TelegramReceiveMode }).receive_mode); setModeEditing(true);
  ...
  return <div className={styles.page}>
    ...
    {!data ? !error && <p role="status">正在读取渠道账户…</p> : <>
      ...
      {data.account.provider === "telegram" && <section className={styles.card}><div className={styles.row}><h2>接收方式</h2><StatusBadge tone="gray">{receiveModeLabel(data.account.config.receive_mode)}</StatusBadge></div><p className={styles.hint}>只更改期望配置；保存不会访问 Telegram、启动接收或清空积压。旧接收停止由 Gateway 在下一次启用时协调，不由页面推定完成。</p>{data.account.config.receive_mode === "webhook" && !data.account.credentials.find((c) => c.purpose === "telegram.webhook_secret")?.configured && <p className={styles.warning}>Webhook Secret 尚未配置。模式可保存在停用状态，启用前请补齐。</p>}{access.canWrite && (data.account.enabled ? <><p className={styles.hint}>更改前请先停用接入。路由开启意图保留；不会自动重新启用。</p><Button variant="secondary" disabled={blocked} onClick={() => void openAction({ kind: "account", enabled: false })}>先停用接入</Button></> : modeEditing ? <div className={styles.stack}><ReceiveModeSelector value={nextMode} onChange={setNextMode} disabled={blocked} /><div className={styles.actions}><Button disabled={blocked || nextMode === data.account.config.receive_mode} onClick={() => void openAction({ kind: "mode", mode: nextMode })}>检查并保存接收方式</Button><Button variant="secondary" disabled={busy} onClick={() => setModeEditing(false)}>取消更改接收方式</Button></div></div> : <Button variant="secondary" disabled={blocked || !isTelegramReceiveMode(data.account.config.receive_mode)} onClick={() => { if (isTelegramReceiveMode(data.account.config.receive_mode)) { setNextMode(data.account.config.receive_mode); setModeEditing(true); } }}>更改接收方式</Button>)}</section>}
