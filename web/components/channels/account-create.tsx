...
import { channelApi, ChannelApiError, channelError, isChannelUncertain, validateChannelAccount, type ChannelProvider, type CreateAccountInput } from "../../lib/channel-api";
...
type CreateMarker = {
  ...
  input: Omit<CreateAccountInput, "credentials">;
};
...
function AccountCreateForm({ tenantId, query, access }: { tenantId: string; query: string; access: ReturnType<typeof useChannelAccess> }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (marker?.operation === "createAccount" && marker.secret && (marker.input.provider === "telegram" || marker.input.provider === "wecom") && typeof marker.input.provider_account_id === "string" && typeof marker.input.name === "string") {
        const restoredMarker: CreateMarker = { operation: "createAccount", secret: true, key: marker.key, createdAt: marker.createdAt, input: { provider: marker.input.provider, provider_account_id: marker.input.provider_account_id, name: marker.input.name, ...(typeof marker.input.description === "string" ? { description: marker.input.description } : {}) } };
  ...
  function bodyFromForm(): CreateAccountInput {
    const identity = pending?.input ?? { provider, provider_account_id: provider === "telegram" ? physicalID.replace(/^0+/, "") : physicalID, name: name.trim(), description };
    return { ...identity, credentials: identity.provider === "telegram" ? {
      "telegram.bot_token": { action: "replace", value: botToken },
      "telegram.webhook_secret": { action: "replace", value: webhookSecret },
  ...
  function review(event?: FormEvent) {
    ...
    const validation = validateChannelAccount(body);
  ...
  async function create() {
    ...
    try {
      ...
      const result = await channelApi.createAccount(tenantId, body, marker.key);
  ...
  function changeProvider(value: ChannelProvider) {
    setProvider(value); setPhysicalID(""); clearSecrets(); setErrors({}); setError("");
  }
  ...
  return <div className={styles.page}>
    ...
    {access.canWrite && initialized && !createdID && <>
      ...
      <form className={styles.form} onSubmit={review} noValidate>
        ...
        <section className={styles.card} aria-labelledby="channel-credential-title"><h2 id="channel-credential-title">2. 接入凭据</h2><p className={styles.small}>凭据仅用于本账户接入，不放入 Runtime Profile。保存后只显示配置状态，不回显原值。</p>
          <div className={styles.grid}>{provider === "telegram" ? <>
            ...
            <div className={`${styles.secretRow} ${styles.full}`}><label className={styles.field}><span>Webhook Secret</span><input type="password" autoComplete="new-password" value={webhookSecret} disabled={secretLocked} aria-invalid={fieldInvalid("credentials.telegram.webhook_secret")} onChange={(e) => setWebhookSecret(e.target.value)} /><small>1～256 位 ASCII 字母、数字、下划线或连字符；与 Bot Token 是两项独立凭据。</small>{fieldError("credentials.telegram.webhook_secret")}</label><Button type="button" variant="secondary" disabled={secretLocked || !!pending} onClick={generateWebhookSecret}>生成随机值</Button></div>
        ...
        <div className={styles.banner}><strong>创建不会启用机器人。</strong>保存后还需选择已发布的 Deployment rN，再分别启用接入与消息路由。当前 Provider、Bot ID 和连接 config 由身份确定，创建后只读。</div>
    ...
    {confirming && reviewedBody.current && <ChannelDialog title={pending ? "确认重放原创建请求" : "确认创建渠道账户"} busy={busy} disabled={!acknowledged} confirmLabel={pending ? "使用原请求标识确认" : "确认创建并保持停用"} onConfirm={() => void create()} onClose={() => { setConfirming(false); reviewedBody.current = null; }}>
      <dl className={styles.summary}><dt>渠道类型</dt><dd>{reviewedBody.current.provider === "telegram" ? "Telegram" : "企业微信"}</dd><dt>稳定机器人 ID</dt><dd>{reviewedBody.current.provider_account_id}</dd><dt>账户名称</dt><dd>{reviewedBody.current.name}</dd><dt>创建后的接入配置</dt><dd><StatusBadge tone="gray">停用</StatusBadge></dd><dt>消息运行目标</dt><dd>尚未创建，不自动启用</dd></dl>
