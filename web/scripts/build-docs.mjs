...
const output = path.join(root, "web/public/docs");
...
for (const slug of files) {
  ...
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Agent tRPC V1 使用参考：${escape(titles[slug])}"><title>${escape(titles[slug])} · Agent tRPC 文档</title><style>${css}</style></head><body><a class="skip" href="#main-content">跳到正文</a><header><a class="brand" href="/">⬡ Agent <strong>tRPC</strong></a><nav aria-label="站点导航"><a href="/docs/guide.html">教程</a><a href="/docs">文档中心</a><a class="console" href="/console">进入控制台 ↗</a></nav></header><div class="layout"><aside><a class="back" href="/docs">← 返回文档中心</a><span class="label">REFERENCE / V1</span><nav aria-label="参考文档">${nav}</nav><span class="label">本文目录</span><nav aria-label="本文目录">${toc.join("")}</nav></aside><main id="main-content"><div class="eyebrow">AGENT TRPC / USER REFERENCE</div><h1>${escape(titles[slug])}</h1><p class="meta">V1 · 使用者参考 · 2026.09.07</p><details><summary>文档与本页目录</summary><nav>${nav}${toc.join("")}</nav></details><article>${body}</article><footer><a href="/docs">← 返回文档中心</a><a href="/docs/guide.html">阅读完整图文教程 →</a></footer></main></div></body></html>`;
  writeArtifact(path.join(output, "reference", `${slug}.html`), html);
  manifest.pages.push({ url: `/docs/reference/${slug}.html`, source: `docs/site/${slug}.md`, sha256: createHash("sha256").update(markdown).digest("hex") });
}
writeArtifact(path.join(output, "guide.html"), renderUserGuide({ siteNavigation: true }));
manifest.pages.push({ url: "/docs/guide.html", source: "docs/user-guide/v1/README.md", sha256: createHash("sha256").update(fs.readFileSync(path.join(root, "docs/user-guide/v1/README.md"))).digest("hex") });
