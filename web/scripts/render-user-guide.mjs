...
export function renderUserGuide({ siteNavigation = false } = {}) {
  const navigation = siteNavigation
    ? `<header class="site-header"><a class="brand" href="/" aria-label="Agent tRPC 主页">${logo}</a><nav class="site-nav" aria-label="主导航"><a href="/">概览</a><a href="/docs/guide.html" aria-current="page">使用教程</a><a href="/docs">文档</a></nav><a class="nav-cta" href="/console">进入控制台 <span aria-hidden="true">→</span></a></header>`
...
const page = renderUserGuide();
const destination = path.join(dir, '部署与使用指南.html');
if (process.argv.includes('--check')) {
  if (fs.readFileSync(destination, 'utf8') !== page) throw new Error('User guide HTML is stale. Run npm run docs:sync.');
} else fs.writeFileSync(destination, page);
console.log(`HTML generated: ${page.length} characters; headings=${headings.length}; embedded images=${(page.match(/data:image\//g) || []).length}`);
