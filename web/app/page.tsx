...
export default function Home() {
  return <SiteShell active="home"><main id="main-content">
    <section className={styles.hero} aria-labelledby="hero-title"><div className={styles.heroCopy}><a className={styles.releaseBadge} href={`${guideUrl}#chapter-1`}><span>V1</span> 从搭建到接入，一条完整路径 <ArrowUpRight size={14} /></a><h1 id="hero-title">让你的 Agent，<br /><span>真正开始工作。</span></h1><p className={styles.heroDescription}>定义它的角色，连接你的模型。<br />把经过校验的版本，带到用户的下一次对话里。</p><div className={styles.heroActions}><a className={styles.primaryButton} href={`${guideUrl}#chapter-4`}>开始搭建我的 Agent <ArrowRight size={17} /></a><a className={styles.secondaryButton} href="/docs"><BookOpen size={18} />阅读文档</a></div><div className={styles.heroFootnote}><span><Check size={14} />可视化配置</span><span><Check size={14} />版本化发布</span><span><Check size={14} />真实渠道接入</span></div><a className={styles.scrollCue} href="#start-here"><span>从这里开始探索</span><ArrowDown size={15} /></a></div><WorkflowPreview /></section>
    ...
    <section className={styles.section} id="start-here" aria-labelledby="tutorial-title"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>FROM ZERO TO YOUR FIRST REPLY</span><h2 id="tutorial-title">不用猜下一步，<br className={styles.mobileBreak} />跟着做就好。</h2></div><a className={styles.textLink} href={guideUrl}>全部使用教程 <ArrowRight size={16} /></a></div><div className={styles.tutorialGrid}>
      <a className={styles.featuredTutorial} href={`${guideUrl}#chapter-4`}><span className={styles.tutorialLabel}><BookOpen size={16} />新手完整教程</span><h3>部署你的<br />第一个 Agent</h3><p>从一个 Single LLM 助手开始，<br />直到机器人里出现第一条真实回复。</p><div className={styles.miniSteps}><span><Bot size={19} />创建</span><ArrowRight size={16} /><span><Settings2 size={19} />配置</span><ArrowRight size={16} /><span><Rocket size={19} />发布</span><ArrowRight size={16} /><span><MessageCircle size={19} />对话</span></div><div className={styles.tutorialBottom}><span>图文指南 · 逐步操作</span><span className={styles.roundArrow}><ArrowUpRight size={23} /></span></div></a>
      <div className={styles.sideTutorials}><a className={styles.smallTutorial} href={`${guideUrl}#chapter-13`}><span className={styles.tutorialIcon}><Terminal size={24} /></span><span className={styles.docTag}>安装与准备</span><h3>先拥有自己的平台</h3><p>环境准备、服务启动、首次登录。<br />从这里搭好工作的起点。</p><span className={styles.textLink}>阅读部署指南 <ArrowUpRight size={17} /></span></a><a className={styles.smallTutorial} href={`${guideUrl}#chapter-7`}><span className={styles.tutorialIcon}><Send size={24} /></span><span className={styles.docTag}>TELEGRAM 接入</span><h3>没有公网 IP，也能开始</h3><p>使用长轮询，完成接入预检，<br />再验证一次真正的收发。</p><span className={styles.textLink}>连接我的机器人 <ArrowUpRight size={17} /></span></a></div>
    </div></section>
    ...
    <section className={styles.section} aria-labelledby="docs-title"><div className={styles.sectionHeading}><div><span className={styles.eyebrow}>A PLACE FOR EVERY ANSWER</span><h2 id="docs-title">需要时，文档就在这里。</h2></div><a className={styles.textLink} href="/docs">进入文档中心 <ArrowRight size={16} /></a></div><DocCards /></section>
    <section className={styles.finalCta}><span className={styles.ctaSymbol}><Sparkles size={33} strokeWidth={1.3} /></span><div><h2>你的下一个助手，从这里开始。</h2><p>先完成一个简单、可用的 Agent，再逐步打磨它。</p></div><a className={styles.primaryButton} href="/console">进入控制台 <ArrowRight size={17} /></a></section>
    <aside className={styles.scopeNote}>V1 执行路径：单 LLM + 模型 + session 会话存储 + 文本回复。工具、知识与组合节点的配置入口不代表当前 Worker 已支持执行。<a href={`${guideUrl}#chapter-1`}>了解当前版本 <ArrowUpRight size={13} /></a></aside>
  </main></SiteShell>;
}
