...
export default function AuditPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(undefined);
    try {
      const page = await controlApi.listAuditEvents(tenantId, { offset, limit: PAGE_SIZE });
    ...
    <div className="panel">
      ...
      {!loading && total > 0 && <div className="pagination">
        ...
        <div className="pagination-actions">
          <Button disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - PAGE_SIZE))} variant="secondary">上一页</Button>
          <Button disabled={offset + PAGE_SIZE >= total || offset + PAGE_SIZE >= V1_MERGE_WINDOW} onClick={() => setOffset(offset + PAGE_SIZE)} variant="secondary">下一页</Button>
        </div>
      </div>}
    </div>
  </>;
