...
export default function RunDetailPage() {
  ...
  return <>
    ...
    {!run ? (!error && <div className="panel-loading"><div className="loading-mark"/><span>正在读取运行详情…</span></div>) : <>
      <div className="stats-grid"><StatCard label="当前阶段" value={run.stage} detail={run.status}/><StatCard label="Attempt" value={run.attempts} detail={run.failure_reason || "无持久化失败原因"}/><StatCard label="Token" value={run.total_tokens.toLocaleString("zh-CN")} detail={`${run.input_tokens} 输入 / ${run.output_tokens} 输出`}/><StatCard label="回复" value={run.reply_status} detail={run.memory_status || "无 Memory 阻塞"}/></div>
